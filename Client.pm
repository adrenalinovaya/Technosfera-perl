...
sub _parse {
    ...
    if (defined $self->conf) {
        $self->conf->{content} = decode($self->conf->{cp}, $self->conf->{content});
...
sub get_links {
    ...
    while (my $token = $p->get_token())
    {
        ...
        if ($token->[0] eq 'S' && 
            ...
            $token->[2]->{href} =~ m{^https://habrahabr.ru/post/.*#habracut$}
            )
        {
...
sub parse_new{
    ...
    while (my $tok = $iter->get_next()) {
        ...
        ) 
        {
            ...
            $tok->[1] eq 'div' &&
            defined $tok->[2]->{class} &&
            $tok->[2]->{class} =~ m/(statistic__value statistic__value_magenta)|(user-rating__value)/
            )
        {
            ...
            )
        {
            ...
            my $nick = $stat->{user}{nick}; $nick =~ s/@//gs;
