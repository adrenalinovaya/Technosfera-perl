...
sub add_user {
	...
	my $sql_usr = 'INSERT INTO `users` (nick, rating, karma, href) VALUES (?, ?, ?, ?)';

	my $sth = $self->dbh->prepare($sql_usr) or die $self->dbh->errstr;
	eval {
		$sth->execute($user->{nick}, $user->{rating}, $user->{karma}, $user->{href}) or die $sth->errstr;	
		$self->users->{$user->{nick}} = {id => $sth->{mysql_insertid}, %{$user}};
...
sub add_article {
	...
	my $sql_dat;
	my $keys = [$art->{href}, $art->{title}, $art->{views}, $art->{stars}, $art->{user_id}];
	if (defined $self->articles->{$art->{href}}) {
		$sql_dat = 'UPDATE `data` SET href=?, title=?, views=?, stars=?, user_id=? WHERE href=?';
		push @$keys, $art->{href};
	} else {
		$sql_dat = 'INSERT INTO `data` (href, title, views, stars, user_id) VALUES (?, ?, ?, ?, ?)';
	}
...
sub update_commenter {
	...
	my $sql_com = 'UPDATE `commenters` SET comment_count = ? WHERE post_id = ? and user_id = ?';
	my $sth = $self->dbh->prepare($sql_com) or die $self->dbh->errstr;
	eval {
		$sth->execute($com->{comment_count}, $com->{post_id}, $com->{user_id}) or
			die $sth->errstr;
		1;
	};
	if ($@) {
		warn $sth->errstr . "\n";
		warn 'first comment from user_id ' . $com->{user_id} . ' in this article' . "\n";
		return undef;
	}
	1;
}
...
sub add_commenters {
	...
	my $sql_com = 'INSERT INTO `commenters` (post_id, user_id, comment_count) VALUES (?, ?, ?)';
...
sub update_user {
	...
	my $sql_usr = 'UPDATE `users` SET nick=?, rating=?, karma=?, href=? WHERE id=?';

	my $sth = $self->dbh->prepare($sql_usr) or die $self->dbh->errstr;
	eval {
		$sth->execute($user->{nick}, $user->{rating}, $user->{karma}, $user->{href}, $user->{id}) 
			or die $sth->errstr;	
		$self->users->{$user->{nick}} = {id => $sth->{mysql_insertid}, %{$user}};
...
sub cache_users {
	...
	$self->users($self->dbh->selectall_hashref('SELECT * FROM `users`', ['nick']));

	for my $nick (keys %{$self->users}) {
		unless (defined $self->users->{$nick}{href}) {
			$self->update_user({ %{$self->users->{$nick}}, href => Habr->new()->get_href_by_nick($nick) });
		}
	}
...
sub cache_articles {
	...
	$self->articles($self->dbh->selectall_hashref(
		'SELECT * FROM `data`', ['href']));
...
sub select_user {
	...
	my $usr = $self->dbh->selectall_hashref
	(
...
sub get_post {
	...
	my $uid = $self->get_user($info->{user}{nick}, $refresh, $info->{user}{href})->{id};
	$self->add_article({ %{$info->{article}}, user_id => $uid }, $info->{user}, $refresh);
...
sub get_commenters {
	...
	my $sql_com = 'SELECT * FROM `commenters` as c JOIN `users` as u on c.user_id = u.id WHERE post_id=?';
...
sub self_commenters {
	...
	my $sql_self =  'SELECT * FROM users as u WHERE id IN ' . 
...
sub desert_posts {
	...
	my $sql_desert =  	'SELECT * FROM data as d WHERE ' .
						'(SELECT SUM(comment_count) FROM commenters as c ' .
	                	'WHERE c.post_id = d.id) < ?';
	my $posts = $self->dbh->selectall_hashref($sql_desert, ['href'], {}, $min_count);
	return $posts;
}
